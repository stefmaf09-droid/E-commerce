...
CREATE TABLE IF NOT EXISTS clients (
    ...
    stripe_connect_id TEXT UNIQUE, -- ID Stripe Connect (acct_...)
    stripe_onboarding_status TEXT DEFAULT 'pending', -- pending, restricted, active
    subscription_tier TEXT DEFAULT 'standard', -- standard, business, enterprise
    commission_rate REAL DEFAULT 20.0 -- Pourcentage de commission (Success Fee)
);
...
CREATE TABLE IF NOT EXISTS stores (
    ...
    store_url TEXT,
    country TEXT DEFAULT 'FR', -- FR, US, UK, etc.
    currency TEXT DEFAULT 'EUR', -- EUR, USD, GBP, etc.
    is_active BOOLEAN DEFAULT 1,
    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
...
CREATE TABLE IF NOT EXISTS claims (
    id INTEGER PRIMARY KEY AUTOINCREMENT,
    claim_reference TEXT UNIQUE NOT NULL, -- CLM-20260125-XXXX
    client_id INTEGER NOT NULL,
    store_id INTEGER,
    order_id TEXT NOT NULL,
    carrier TEXT NOT NULL,
    dispute_type TEXT NOT NULL, -- late_delivery, lost, damaged, invalid_pod
    amount_requested REAL NOT NULL,
    currency TEXT DEFAULT 'EUR',
    status TEXT DEFAULT 'pending', -- pending, submitted, under_review, accepted, rejected, paid
    ...
    success_probability REAL, -- Score de 0 à 1 au moment du dépôt
    ...
    -- Automation
    skill_used TEXT, -- chronopost_claim, colissimo_claim, etc.
    automation_status TEXT, -- manual, automated, failed
    ...
    follow_up_level INTEGER DEFAULT 0, -- 0: none, 1: status_request, 2: warning, 3: formal_notice
...
CREATE TABLE IF NOT EXISTS disputes (
    ...
    claim_id INTEGER, -- NULL si pas encore réclamé
    is_claimed BOOLEAN DEFAULT 0,
    success_probability REAL, -- Score de 0 à 1 (AI Predictor)
    predicted_days_to_recovery INTEGER, -- Délai estimé via IA
...
CREATE TABLE IF NOT EXISTS payments (
    ...
    client_share REAL NOT NULL, -- 80%
    platform_fee REAL NOT NULL, -- 20%
    commission_amount REAL, -- Montant exact prélevé (peut varier si promo/frais)
    currency TEXT DEFAULT 'EUR',
    payment_method TEXT, -- stripe_connect, manual_transfer
...
CREATE TABLE IF NOT EXISTS notifications (
    id INTEGER PRIMARY KEY AUTOINCREMENT,
    client_id INTEGER NOT NULL,
    notification_type TEXT NOT NULL, -- welcome, claim_submitted, claim_accepted, disputes_detected
...
CREATE TABLE IF NOT EXISTS activity_logs (
    id INTEGER PRIMARY KEY AUTOINCREMENT,
    client_id INTEGER,
    action TEXT NOT NULL, -- login, claim_submitted, data_exported, etc.
    resource_type TEXT, -- claim, payment, store
    resource_id INTEGER,
    ip_address TEXT,
    user_agent TEXT,
    details TEXT, -- JSON
...
CREATE TABLE IF NOT EXISTS system_alerts (
    id INTEGER PRIMARY KEY AUTOINCREMENT,
    alert_type TEXT NOT NULL, -- bypass_detected, api_error, payment_issue
...
CREATE TABLE IF NOT EXISTS global_fraud_registry (
    id INTEGER PRIMARY KEY AUTOINCREMENT,
    entity_type TEXT NOT NULL, -- email, phone, address, ip
...
INSERT OR IGNORE INTO system_settings (key, value, description) VALUES
    ('platform_commission_rate', '0.20', 'Taux de commission plateforme (20%)'),
    ('email_digest_enabled', '1', 'Activer emails digest quotidiens'),
    ('auto_submit_claims', '0', 'Soumission automatique des réclamations'),
    ('stripe_mode', 'test', 'Mode Stripe (test ou live)');
...
SELECT 
    ...
    SUM(CASE WHEN cl.status = 'accepted' THEN 1 ELSE 0 END) as accepted_claims,
    SUM(CASE WHEN cl.status = 'rejected' THEN 1 ELSE 0 END) as rejected_claims,
    SUM(CASE WHEN cl.status = 'pending' THEN 1 ELSE 0 END) as pending_claims,
    SUM(cl.amount_requested) as total_requested,
    SUM(CASE WHEN cl.status = 'accepted' THEN cl.accepted_amount ELSE 0 END) as total_recovered,
    SUM(CASE WHEN p.payment_status = 'completed' THEN p.client_share ELSE 0 END) as total_paid_to_client,
